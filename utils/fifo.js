...
export function consumeFIFO(item, requestedQuantity) {
  ...
  if (item.total_qty < remaining) {
    throw new Error(`Insufficient inventory. Available: ${item.total_qty}`);
  }
  ...
  return {
    cogs: consumed.reduce((sum, row) => sum + row.quantity * row.unit_cost, 0),
    consumed
  };
}
