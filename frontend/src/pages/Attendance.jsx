...
export default function Attendance() {
  ...
  async function remove(id) {
    ...
    try {
      ...
      setToast('রেকর্ড মুছে ফেলা হয়েছে');
      await load('attendance', '/attendance');
    } catch (error) {
      setToast(error.message);
    }
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <>
            ...
            <Button icon={Plus} onClick={() => openForm()}>নতুন এন্ট্রি</Button>
          </>
        }
      />

      <section className="attendance-highlight panel">
        <div className="section-title-row">
          <div>
            <h3>আজকের হাজিরা <span>{today}</span><i>লাইভ</i></h3>
          </div>
        </div>

        <div className="attendance-summary">
          <span><b className="green">{countStatus('Present')}</b>উপস্থিত</span>
          <span><b className="amber">{countStatus('Late')}</b>দেরি</span>
          <span><b className="danger">{countStatus('Absent')}</b>অনুপস্থিত</span>
          <span><b className="amber">{countStatus('On Leave')}</b>ছুটি</span>
          <span><b>{employees.length - todayRecords.filter(Boolean).length}</b>রেকর্ড নেই</span>
          <span><b>{employees.length}</b>মোট</span>
        </div>
        ...
        <div className="attendance-people">
          {employees.map((employee, index) => {
            ...
            return (
              <div className="person-card" key={employee._id}>
                <div className="person-avatar">{employee.name.slice(0, 2).toUpperCase()}</div>
                <div>
                  <b>{employee.name}</b>
                  <small>{employee.designation}</small>
                  {record?.check_in && <small>{record.check_in}</small>}
                </div>
                <span className={`status ${record ? `status-${record.status.toLowerCase().replace(' ', '-')}` : ''}`}>
                  {record?.status || 'রেকর্ড নেই'}
                </span>
              </div>
            );
          })}
      ...
      <section className="date-attendance panel">
        <div className="section-title-row">
          <div>
            <h3>তারিখ অনুযায়ী হাজিরা <i>প্রিমিয়াম</i></h3>
            <p>নির্দিষ্ট তারিখের উপস্থিতি দেখুন</p>
          </div>
          <div className="date-actions">
            <input type="date" value={selectedDate} onChange={event => setSelectedDate(event.target.value)} />
            <Button onClick={() => setDateResults(dateRecords)}>দেখুন</Button>
            <Button variant="outline" onClick={() => { setDateResults(null); setSelectedDate(today); }}>রিসেট</Button>
          </div>
        </div>

        {dateResults ? (
          <div className="date-result-grid">
            {dateRecords.map((record, index) => (
              <div className="date-result" key={employees[index]?._id || index}>
                <div>
                  <b>{employees[index]?.name}</b>
                  <small>{employees[index]?.designation}</small>
                </div>
                <span className={`status ${record ? `status-${record.status.toLowerCase().replace(' ', '-')}` : ''}`}>
                  {record?.status || 'রেকর্ড নেই'}
                </span>
          ...
          <div className="date-empty">
            <Clock3 size={30} />
            <b>একটি তারিখ নির্বাচন করে দেখুন</b>
            <span>কে উপস্থিত ছিল তার তালিকা দেখাবে</span>
          </div>
        )}
      </section>
      ...
      <div className="attendance-history">
        <div className="attendance-history-head">
          ...
          <div className="history-search">
            <Toolbar value={nameFilter} onChange={setNameFilter} placeholder="কর্মীর নাম দিয়ে খুঁজুন..." />
        ...
        {loading ? (
          ...
          <Empty text="কোন হাজিরা রেকর্ড পাওয়া যায়নি" />
        ) : (
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {filtered.map(record => (
                    <tr key={record._id}>
                      <td><b>{record.employee_name}</b></td>
                      <td>{new Date(record.date).toLocaleDateString('bn-BD')}</td>
                      <td>{record.check_in || '--'}</td>
                      <td>{record.check_out || '--'}</td>
                      <td><span className={`status status-${record.status.toLowerCase().replace(' ', '-')}`}>{record.status}</span></td>
                      <td>
                        <div className="row-actions">
                          <button className="icon-btn" onClick={() => openForm(record)}><Pencil size={16} /></button>
                          <button className="icon-btn danger-btn" onClick={() => remove(record._id)}><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {modal && (
        <Modal title={modal._id ? 'হাজিরা সম্পাদনা' : 'নতুন হাজিরা'} onClose={() => setModal(null)}>
          <form onSubmit={save} className="form-grid">
            <FormField label="কর্মী নাম">
              <select required value={form.employee_id || ''} onChange={event => {
                const employee = employees.find(item => item._id === event.target.value);
                setForm({ ...form, employee_id: employee?._id, employee_name: employee?.name });
              }}>
            ...
            <FormField label="চেক-ইন">
              <input type="time" value={form.check_in || ''} onChange={event => setForm({ ...form, check_in: event.target.value })} />
            </FormField>

            <FormField label="চেক-আউট">
              <input type="time" value={form.check_out || ''} onChange={event => setForm({ ...form, check_out: event.target.value })} />
            </FormField>

            <FormField label="স্ট্যাটাস">
              <select value={form.status || 'Present'} onChange={event => setForm({ ...form, status: event.target.value })}>
                {statuses.map(status => (
