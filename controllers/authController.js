...
export const login = asyncHandler(async (req, res) => {
  ...
  if (user.role === 'admin' && user.twoFactorEnabled) {
    const validTotp = req.body.twoFactorCode &&
      authenticator.check(req.body.twoFactorCode, user.twoFactorSecret);
    
    const backupIndex = user.backupCodes?.findIndex(code =>
      code === hash(req.body.twoFactorCode || '')
    );
